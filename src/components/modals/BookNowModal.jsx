...
import { X, Plus, Trash2, MapPin, Mountain, Package, IndianRupee, Users, ChevronDown, Check } from "lucide-react";
...
const BookNowModal = ({ isOpen, onClose, trekData }) => {
  ...
  const bookingType = typeof trekData?.bookingType === 'object'
  ...
  const trekPickupPoints = (trekData?.pickup?.length ? trekData.pickup : infoMap["PICKUP"]) || [];
  const trekDropPoints = (trekData?.dropoff?.length ? trekData.dropoff : infoMap["DROPOFF"]) || [];
  const trekSuitableFor = (trekData?.suitableFor?.length ? trekData.suitableFor : infoMap["SUITABLE FOR"]) || [];
  const trekAccommodation = (trekData?.accommodation?.length ? trekData.accommodation : infoMap["ACCOMMODATION"]) || [];
  const trekTitle = trekData?.title || "";

  // Available Add-ons for this trek
  const availableAddons = useMemo(() => {
    // If trekData has addons array, use it
    if (trekData?.addons && Array.isArray(trekData.addons) && trekData.addons.length > 0) {
  ...
  const priceBreakdown = useMemo(() => {
    ...
    const allAddonsTotal = availableAddons.reduce((sum, a) => sum + (a.price || 0), 0);

    // Price of currently selected add-ons
    const selectedAddonsTotal = formData.addons.reduce((sum, a) => sum + (a.price || 0), 0);

    // Amount to subtract from the "full package" if items are deselected
    const deductions = allAddonsTotal - selectedAddonsTotal;

    const packageSubtotal = baseTotal + allAddonsTotal;
    const subtotal = baseTotal + selectedAddonsTotal;

    const discountAmount = discountPercent > 0 ? Math.round((baseTotal * discountPercent) / 100) : 0;
    const grandTotal = subtotal - discountAmount;

    return { numPeople, baseTotal, allAddonsTotal, selectedAddonsTotal, deductions, packageSubtotal, subtotal, discountAmount, grandTotal };
  }, [formData.numberOfPeople, formData.addons, availableAddons, totalFee, discountPercent]);
  ...
  const handleAddonToggle = (addon) => {
    setFormData((prev) => {
      ...
      if (exists) {
        return { ...prev, addons: prev.addons.filter((a) => a.name !== addon.name) };
      } else {
        return { ...prev, addons: [...prev.addons, { name: addon.name, price: addon.price }] };
      }
    });
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.alternativeContact.trim()) newErrors.alternativeContact = "Alternative contact is required";
    if (!formData.emergencyContact.trim()) newErrors.emergencyContact = "Emergency contact is required";
    if (!formData.departureDate) newErrors.departureDate = "Departure date is required";
    if (formData.numberOfPeople < 1) newErrors.numberOfPeople = "Must be at least 1 person";
      ...
      if (formData.additionalMembers.length < requiredMembers) {
        ...
        formData.additionalMembers.forEach((member, index) => {
          ...
          if (!member.whatsappNumber?.trim()) {
            newErrors[`member_${index}_whatsapp`] = "Member WhatsApp is required";
          }
        });
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formattedAdditionalMembers = formData.additionalMembers.map((member) => ({
  ...
  const labelCls = "text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5 block";
          ...
          <div className="overflow-y-auto custom-scrollbar flex-1">
            ...
            <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 px-8 py-8 rounded-t-3xl">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-blue-200 text-xs font-semibold uppercase tracking-widest">Booking For</p>
                  <h2 className="text-xl font-bold text-white leading-tight">{trekTitle}</h2>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      <input type="text" name="name" value={formData.name} onChange={handleChange}
                        placeholder="Enter your full name" className={inputCls(errors.name)} />
                    ...
                    <div>
                      ...
                      <input type="tel" name="whatsappNumber" value={formData.whatsappNumber} onChange={handleChange}
                        placeholder="Enter your WhatsApp number" className={inputCls(errors.whatsappNumber)} />
                    ...
                    <div>
                      ...
                      <select name="gender" value={formData.gender} onChange={handleChange} className={selectCls(errors.gender)}>
                    ...
                    <div>
                      ...
                      <select name="bloodGroup" value={formData.bloodGroup} onChange={handleChange} className={selectCls(errors.bloodGroup)}>
                ...
                <div>
                  <div className="flex items-center gap-2.5 mb-5">
                    ...
                    <h3 className="text-lg font-bold text-gray-800">Logistics & Preferences</h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Pickup */}
                    <div className="space-y-4">
                      <label className={labelCls}>Pickup Point <span className="text-red-400">*</span></label>
                      <div className="grid gap-2">
                        {trekPickupPoints.length > 0 ? (
                          trekPickupPoints.map((point, i) => (
                            <label key={i} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${formData.pickupPoint === point ? "border-blue-500 bg-blue-50" : "border-gray-100 bg-gray-50/30 hover:bg-gray-50"}`}>
                              <input type="radio" name="pickupPoint" value={point} checked={formData.pickupPoint === point} onChange={handleChange} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                              <span className="text-sm text-gray-700">{point}</span>
                            </label>
                          ))
                        ) : (
                          <input type="text" name="pickupPoint" value={formData.pickupPoint} onChange={handleChange} placeholder="Enter pickup location" className={inputCls(errors.pickupPoint)} />
                        )}
                      </div>
                      ...
                      <div className="grid gap-2">
                        {trekDropPoints.length > 0 ? (
                          trekDropPoints.map((point, i) => (
                            <label key={i} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${formData.dropPoint === point ? "border-emerald-500 bg-emerald-50" : "border-gray-100 bg-gray-50/30 hover:bg-gray-50"}`}>
                              <input type="radio" name="dropPoint" value={point} checked={formData.dropPoint === point} onChange={handleChange} className="w-4 h-4 text-emerald-600 focus:ring-emerald-500" />
                              <span className="text-sm text-gray-700">{point}</span>
                            </label>
                          ))
                        ) : (
                          <input type="text" name="dropPoint" value={formData.dropPoint} onChange={handleChange} placeholder="Enter drop location" className={inputCls(false)} />
                        )}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="md:col-span-2">
                      ...
                      <textarea name="medicalHistory" value={formData.medicalHistory} onChange={handleChange}
                        placeholder="Any allergies, conditions, or medications..." rows="3"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="text" name="emergencyContactName" value={formData.emergencyContactName} onChange={handleChange}
                        placeholder="Contact person name" className={inputCls(false)} />
                    </div>
                    <div>
                      <label className={labelCls}>Emergency Contact Number <span className="text-red-400">*</span></label>
                      <input type="tel" name="emergencyContact" value={formData.emergencyContact} onChange={handleChange}
                        placeholder="Emergency contact number" className={inputCls(errors.emergencyContact)} />
                      {errors.emergencyContact && <p className="text-red-500 text-xs mt-1 ml-1">{errors.emergencyContact}</p>}
                    </div>
                    <div>
                      <label className={labelCls}>Alternative Contact Number <span className="text-red-400">*</span></label>
                      <input type="tel" name="alternativeContact" value={formData.alternativeContact} onChange={handleChange}
                        placeholder="Alternative contact number" className={inputCls(errors.alternativeContact)} />
                      {errors.alternativeContact && <p className="text-red-500 text-xs mt-1 ml-1">{errors.alternativeContact}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className={labelCls}>Departure Date <span className="text-red-400">*</span></label>
                      ...
                      {errors.departureDate && <p className="text-red-500 text-xs mt-1 ml-1">{errors.departureDate}</p>}
                    </div>
                    <div>
                      <label className={labelCls}>Number of People <span className="text-red-400">*</span></label>
                      <input type="number" name="numberOfPeople" value={formData.numberOfPeople} onChange={handleChange}
                        min="1" max={trekData?.groupSize?.split("-")[1] || "20"}
                        className={inputCls(errors.numberOfPeople)} />
                      {errors.numberOfPeople && <p className="text-red-500 text-xs mt-1 ml-1">{errors.numberOfPeople}</p>}
                    </div>
                    ...
                    {bookingType === "Trek + Camping" && (
                      <div className="md:col-span-2">
                        <label className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 bg-gray-50/50 cursor-pointer hover:border-blue-200 hover:bg-blue-50/30 transition-all">
                          <input type="checkbox" name="needCoupleTent" checked={formData.needCoupleTent} onChange={handleChange}
                            className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                          <span className="text-sm font-medium text-gray-700">Need Couple Tent</span>
                        </label>
                      </div>
                    )}
                    ...
                    {bookingType === "Trip" && (
                      <div className="md:col-span-2">
                        <label className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 bg-gray-50/50 cursor-pointer hover:border-blue-200 hover:bg-blue-50/30 transition-all">
                          <input type="checkbox" name="needPrivateRoom" checked={formData.needPrivateRoom} onChange={handleChange}
                            className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                          <span className="text-sm font-medium text-gray-700">Need Couple/Private Room Accommodation</span>
                        </label>
                      </div>
                    )}
                  </div>
                ...
                {availableAddons.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2.5 mb-5">
                      ...
                      <span className="text-xs text-gray-400 font-medium ml-auto">Select to enhance your experience</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {availableAddons.map((addon, i) => {
                        const isSelected = formData.addons.some((a) => a.name === addon.name);
                        return (
                          <button key={i} type="button" onClick={() => handleAddonToggle(addon)}
                          ...
                          >
                            ...
                            <span className={`text-sm font-bold whitespace-nowrap ${isSelected ? "text-purple-600" : "text-gray-500"}`}>
                ...
                {showAdditionalMembers && (
                  <div>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <h3 className="text-lg font-bold text-gray-800">
                          ...
                          <span className="text-sm font-normal text-gray-400 ml-2">
                            ({formData.additionalMembers.length}/{requiredAdditionalMembers})
                          </span>
                        </h3>
                      </div>
                      {formData.additionalMembers.length < requiredAdditionalMembers && (
                    ...
                    {errors.additionalMembers && (
                      <p className="text-red-500 text-sm mb-4 bg-red-50 p-3 rounded-xl">{errors.additionalMembers}</p>
                    )}

                    <div className="space-y-4">
                      {formData.additionalMembers.map((member, index) => (
                        <div key={index} className="p-5 bg-gray-50/80 rounded-2xl border border-gray-200 relative">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <label className={labelCls}>Full Name <span className="text-red-400">*</span></label>
                              <input type="text" value={member.fullName || ""} placeholder="Full Name"
                                onChange={(e) => handleMemberChange(index, "fullName", e.target.value)}
                                className={inputCls(errors[`member_${index}_fullName`])} />
                              {errors[`member_${index}_fullName`] && <p className="text-red-500 text-xs mt-1">{errors[`member_${index}_fullName`]}</p>}
                            </div>

                            <div>
                              <label className={labelCls}>WhatsApp Number <span className="text-red-400">*</span></label>
                              <input type="tel" value={member.whatsappNumber || ""} placeholder="+91 98765 43210"
                                onChange={(e) => handleMemberChange(index, "whatsappNumber", e.target.value)}
                                className={inputCls(errors[`member_${index}_whatsapp`])} />
                              {errors[`member_${index}_whatsapp`] && <p className="text-red-500 text-xs mt-1">{errors[`member_${index}_whatsapp`]}</p>}
                            </div>

                            <div>
                              <label className={labelCls}>Contact Number</label>
                              <input type="tel" value={member.contactNumber || ""} placeholder="Contact Number"
                            ...
                            <div>
                              ...
                              <input type="email" value={member.email || ""} placeholder="Email Address"
                            ...
                            <div>
                              ...
                              {trekPickupPoints.length > 0 ? (
                                <select value={member.pickupPoint || ""}
                                  onChange={(e) => handleMemberChange(index, "pickupPoint", e.target.value)}
                                  className={selectCls(false)}>
                                  <option value="">Select Pickup</option>
                                  {trekPickupPoints.map((p, pi) => <option key={pi} value={p}>{p}</option>)}
                                </select>
                              ) : (
                                <input type="text" value={member.pickupPoint || ""} placeholder="Pickup Point"
                                  onChange={(e) => handleMemberChange(index, "pickupPoint", e.target.value)}
                                  className={inputCls(false)} />
                              )}
                            </div>
                            ...
                            <div>
                              <label className={labelCls}>Drop Point</label>
                              {trekDropPoints.length > 0 ? (
                                <select value={member.dropPoint || ""}
                                  onChange={(e) => handleMemberChange(index, "dropPoint", e.target.value)}
                                  className={selectCls(false)}>
                                  <option value="">Select Drop</option>
                                  {trekDropPoints.map((p, pi) => <option key={pi} value={p}>{p}</option>)}
                                </select>
                              ) : (
                                <input type="text" value={member.dropPoint || ""} placeholder="Drop Point"
                                  onChange={(e) => handleMemberChange(index, "dropPoint", e.target.value)}
                                  className={inputCls(false)} />
                              )}
                            </div>
                            ...
                            <div>
                              <label className={labelCls}>Medical History</label>
                              <input type="text" value={member.medicalHistory || ""} placeholder="Medical History (if any)"
                            ...
                            <div>
                              ...
                              <select value={member.bloodGroup || ""}
                ...
                <div className="bg-gradient-to-br from-gray-50 to-blue-50/50 rounded-2xl border border-gray-200 p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-semibold text-gray-800">₹{priceBreakdown.baseTotal.toLocaleString()}</span>
                    </div>

                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-600">Full Package Add-ons</span>
                      <span className="font-semibold text-gray-800">+ ₹{priceBreakdown.allAddonsTotal.toLocaleString()}</span>
                    </div>
                    ...
                    {priceBreakdown.deductions > 0 && (
                      <div className="flex justify-between items-center text-sm p-2 bg-red-50 rounded-lg border border-red-100 italic">
                        ...
                        <span className="font-bold text-red-600">- ₹{priceBreakdown.deductions.toLocaleString()}</span>
                      </div>
                    )}
                    ...
                    {priceBreakdown.discountAmount > 0 && (
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-emerald-600 font-medium">Coupon Discount ({discountPercent}%)</span>
                        <span className="font-semibold text-emerald-600">- ₹{priceBreakdown.discountAmount.toLocaleString()}</span>
                      </div>
                    )}

                    <div className="flex justify-between items-center pt-4 border-t-2 border-gray-300">
                      <div>
                        <span className="text-base font-bold text-gray-900 block leading-tight">Total Amount</span>
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Inclusive of all selected taxes</span>
                      </div>
                ...
                <button type="submit"
                  ...
                  Confirm Booking — ₹{priceBreakdown.grandTotal.toLocaleString()}
                </button>
              </form>
            </div>
          </div>
