...
const AddSiderForm = ({ trek }) => {
  ...
  const galleryImages = trek.gallery
    ? trek.gallery.map((img) => img.cdnUrl)
    : [];
  ...
  return (
    <div className="relative max-w-7xl px-4 md:px-6 mb-24 mx-auto grid grid-cols-12 gap-4 mt-8">
      ...
      <div className="col-span-12 lg:col-span-8 relative rounded-lg overflow-hidden h-[500px] shadow-2xl shadow-sky-200/50 group/swiper">
        <Swiper
              ...
              <div className="relative w-full h-full">
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 text-white">
                  <div className="flex items-center gap-3 mb-4 opacity-0 group-hover/swiper:opacity-100 transition-all duration-700 translate-y-4 group-hover/swiper:translate-y-0">
                    <div className="w-5 h-5 rounded-full bg-yellow-400 flex items-center justify-center text-sky-900 shadow-lg shadow-yellow-400/30">
                      <Sparkles size={18} />
                    </div>
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-yellow-300 drop-shadow-md">
                      {s.subtitle}
                    </span>
                  </div>
                  <h3 className="text-2xl md:text-2xl font-black leading-none mb-2 drop-shadow-xl">
                    {s.title}
                  </h3>
        ...
        {gridImages.slice(0, 4).map((img, i) =>
          i === 3 ? (
            <div
              ...
              className="relative cursor-pointer group/node overflow-hidden rounded-lg shadow-xl shadow-sky-100 group-hover/node:shadow-sky-200 transition-all duration-500"
            >
              <img
                src={img}
              ...
              <div className="absolute inset-0 bg-sky-900/40 backdrop-blur-[2px] flex flex-col items-center justify-center text-white gap-2 group-hover/node:bg-sky-600/40 transition-all duration-500">
                <div className="text-4xl font-black text-yellow-400 drop-shadow-lg">
                  +{Math.max(0, galleryImages.length - 3)}
                </div>
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-sky-50">
                  View Gallery
                </div>
              </div>
            ...
            <div
              key={i}
              className="relative group/node overflow-hidden rounded-lg shadow-lg shadow-sky-100/50 hover:shadow-2xl hover:shadow-sky-200/50 transition-all duration-500"
            >
              <img
                src={img}
                className="w-full h-full object-cover transition-transform duration-700 group-hover/node:scale-110"
