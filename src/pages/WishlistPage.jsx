...
import {
  Heart, MapPin, Calendar, Users, Trash2,
  BookmarkPlus, ArrowUpDown, Plane, Star, Globe, TrendingUp,
  Zap, Clock, X, ArrowRight, Sparkles, Mountain, CheckCircle, Eye
} from "lucide-react";
import { useSelector, useDispatch } from "react-redux";
import { Link } from "react-router-dom";
import { fetchWishlistAsync, toggleWishlistAsync, selectWishlistItems } from "../store/slices/wishlistSlice";
...
const WishlistPage = () => {
  ...
  // Extract trek objects from wishlist items
  const wishlistTreks = wishlistItems
    .map(item => item.trek)
    .filter(trek => trek && trek._id);

  const sortedTreks = [...wishlistTreks].sort((a, b) => {
    if (sortBy === "price_low") return (a.feeDetails?.totalFee || 0) - (b.feeDetails?.totalFee || 0);
    if (sortBy === "price_high") return (b.feeDetails?.totalFee || 0) - (a.feeDetails?.totalFee || 0);
    if (sortBy === "rating") return (b.rating || 0) - (a.rating || 0);
    return 0;
  });

  const removeItem = (trekId) => {
    dispatch(toggleWishlistAsync({ trekId, isWishlisted: true }));
  };

  const totalValue = wishlistTreks.reduce((s, i) => s + (i.feeDetails?.totalFee || 0), 0);
  const totalSavings = wishlistTreks.reduce((s, i) => s + (i.feeDetails?.discount?.value || 0), 0);

  const stats = [
    { label: "Saved Treks", value: wishlistTreks.length, desc: "Dream destinations", icon: Heart, gradient: "from-rose-400 to-pink-500", bg: "bg-rose-50", border: "border-rose-100", text: "text-rose-600" },
    ...
    { label: "Saved", value: `${totalSavings}%`, desc: "Total discounts", icon: Zap, gradient: "from-emerald-400 to-teal-500", bg: "bg-emerald-50", border: "border-emerald-100", text: "text-emerald-600" },
  ];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-blue-50 flex items-center justify-center p-6 pt-32">
        <div className="bg-white max-w-md w-full rounded-3xl shadow-2xl border border-sky-100 p-14 text-center">
          ...
          <h2 className="text-2xl font-bold text-sky-900 mb-3">Login Required</h2>
          <p className="text-sky-500/70 text-sm mb-10 leading-relaxed">Please log in to view and manage your wishlist.</p>
          <Link to="/" className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-sky-500 to-blue-500 text-white rounded-xl font-bold shadow-lg shadow-sky-200 hover:shadow-xl transition-all">Go Home</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-20">
        <div className="text-center mb-12" style={{ animation: 'wishFadeUp 0.7s ease-out both' }}>
          <div className="inline-flex items-center gap-2 bg-white border border-rose-200 shadow-sm rounded-full px-4 py-2 mb-6">
            ...
            <span className="bg-rose-100 text-rose-600 text-[10px] font-black px-2 py-0.5 rounded-full">{wishlistTreks.length}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10" style={{ animation: 'wishFadeUp 0.7s ease-out 0.1s both' }}>
          {stats.map((stat, i) => {
        ...
        {wishlistStatus === "loading" && wishlistTreks.length === 0 ? (
            ...
            {wishlistTreks.length > 0 && (
              <div className="bg-white rounded-2xl border border-sky-100 shadow-md shadow-sky-100/40 px-6 py-4
                ...
                <div className="text-sky-400 text-xs font-medium">
                  Showing {wishlistTreks.length} saved treks
                </div>
              </div>
            )}
            ...
            {wishlistTreks.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {sortedTreks.map((trek, idx) => (
                  <div key={trek._id} style={{ animation: `wishFadeUp 0.5s ease-out ${idx * 0.1}s both` }}>
...
const WishlistTrekCard = ({ trek, onRemove }) => {
  ...
  const imageSrc = trek.image?.cdnUrl || trek.image || "https://images.unsplash.com/photo-1551632811-561732d1e306";
    ...
    >
      <div className="relative h-[250px] overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4">
          <span className={`px-3 py-1.5 rounded-xl text-[11px] font-bold uppercase tracking-wide ${diffConfig(trek.difficulty)}`}>
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {[
            ...
            <div key={i} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-sky-50 border border-sky-100 text-[11px] font-semibold text-sky-700">
        ...
        <div className="flex items-center justify-between pt-5 border-t border-sky-100">
          <div>
            <div className="text-[10px] text-sky-400 uppercase tracking-wider mb-0.5 font-bold">From</div>
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-sky-950">₹{(trek.feeDetails?.totalFee || 0).toLocaleString()}</span>
            </div>
          </div>
          <Link to={`/trek/${trek._id}`} className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-xs text-white uppercase tracking-wide
